    ...
    >
      ...
      <div className="p-2 sm:p-3 pt-0">
        ...
        <button 
          ...
          disabled={isLoading(`addToCart-${product.id}`) || (product.stockQuantity !== null && product.stockQuantity <= 0) || product.enabled === false}
        ...
        >
          {isLoading(`addToCart-${product.id}`) ? 'Adding...' : 
           (product.stockQuantity !== null && product.stockQuantity <= 0) ? 'Out of Stock' :
           product.enabled === false ? 'Unavailable' : 'Add to Cart'}
        </button>
  ...
  return (
    <div className="min-h-screen transition-all duration-500 ease-in-out" style={backgroundStyle}>
          ...
          <div className="hidden md:flex justify-between items-center">
            ...
            <div className="flex items-center gap-4">
              ...
              <button 
                ...
                className="px-4 py-2 rounded-xl bg-gradient-to-r from-blue-500 to-indigo-500 text-white font-semibold hover:from-blue-600 hover:to-indigo-600 transition-all shadow-lg"
              >
                My Orders
              </button>
              <button 
      ...
      <div className="px-3 sm:px-6 py-6 sm:py-12">
          ...
          <div className="grid grid-cols-6 gap-2 sm:gap-4 max-w-5xl mx-auto">
            {categories.map((category) => (
              <div key={category.id} className="flex flex-col items-center group cursor-pointer" onClick={() => window.location.href = `/category/${category.id}`}>
                <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-full overflow-hidden border-3 sm:border-4 border-white shadow-lg group-hover:shadow-xl transition-all duration-300 group-hover:scale-110">
                ...
                <h3 className="text-sm sm:text-base font-semibold text-emerald-900 mt-2 sm:mt-3 text-center group-hover:text-emerald-700 transition-colors line-clamp-2 px-1">
                  {category.name}
                </h3>
              </div>
            ))}
        ...
        {/* Products Section - Grouped by Categories */}
        <div>
                  ...
                  return Object.entries(productsByCategory).map(([categoryName, categoryProducts]) => (
                    <div key={categoryName} className="mb-8">
                      <div className="flex items-center justify-between mb-4">
                        <h3 className="text-lg sm:text-xl font-bold text-emerald-800">{categoryName}</h3>
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 z-50 safe-area-pb">
        <div className="grid grid-cols-5 py-2">
          <button 
            onClick={() => setShowProfile(true)}
            className="flex flex-col items-center py-2 px-1 min-h-[60px] justify-center"
          >
          ...
          <button 
            onClick={() => setShowCart(true)}
            className="flex flex-col items-center py-2 px-1 relative min-h-[60px] justify-center"
          >
            <svg className="w-5 h-5 text-gray-600 mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">

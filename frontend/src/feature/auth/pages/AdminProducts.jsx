    ...
    try {
      ...
      if (response.ok) {
        ...
        if (formData.productImages && formData.productImages.length > 0) {
          ...
          await fetch(`${API_URL}/products/${result.id || editingProduct.id}/upload-images`, {
            method: 'POST',
            body: imagesFormData
          });
        }
            ...
            >
              {product.productImageUrl && (
                <img 
                  ...
                  onError={(e) => {
                    console.log('Admin image failed:', e.target.src);
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              >
                ...
                {sellers.map((seller) => (
                  <option key={seller.id} value={seller.email}>{seller.name} ({seller.email})</option>
                ))}
              </select>
              <input
                type="number"
              ...
              <div className="mb-4">
                ...
                <input
                  ...
                  accept="image/*"
                  multiple
                  onChange={(e) => setFormData({...formData, productImages: Array.from(e.target.files)})}
                  className="w-full px-4 py-3 rounded-xl border border-gray-300 focus:outline-none focus:ring-2 focus:ring-purple-500"
                />
                <p className="text-xs text-gray-500 mt-1">Select multiple additional images for this product</p>
              </div>
